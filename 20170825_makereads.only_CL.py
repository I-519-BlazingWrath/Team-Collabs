...
def ProcessCLI(args):#argv is Genomesize, readlength, numberofreads, OPfasta, OPreadsfile
    composition=chooseRandomComposition()
    pp.pprint(composition)
    seq=''
    with open(args[4],'r') as IP:
        for line in IP:
            if line[0]!=">":
                seq+=line
